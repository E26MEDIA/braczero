  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 px-4 pt-4 md:px-6">
      <motion.nav
        ...
        className={`mx-auto flex max-w-6xl items-center justify-between rounded-2xl border px-4 py-3 transition-all duration-500 md:px-5 ${
          scrolled
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            className="mx-auto mt-2 max-w-6xl rounded-2xl border border-white/10 bg-[#0a0e14]/95 p-4 backdrop-blur-xl lg:hidden"
          >
              ...
              <div className="mt-2 border-t border-white/10 pt-2">
                ...
                {serviceLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="block rounded-xl px-3 py-2 text-sm text-muted hover:bg-white/5 hover:text-fg"
                  >
                    {link.label}
                  </Link>
                ))}
